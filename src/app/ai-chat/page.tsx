...
export default function AIChatPage() {
  ...
  const loadAnalysis = async () => {
    try {
      ...
      const response = await api.get(`/ai/analyze?period=${analysisPeriod}`);
      if (response.data.success) {
  ...
  const loadChatHistory = async (sessionId: string) => {
    ...
    try {
      ...
      if (response.data.success) {
        const historyMessages = response.data.data.chats.map((chat: any) => [
  ...
  const handleBudgetRecommendation = async () => {
    ...
    try {
      ...
      const response = await api.post('/ai/budget-recommendation', {
        monthlyIncome: parseFloat(budgetIncome)
      });
      if (response.data.success) {
        setBudgetResult(response.data.data);
      }
    } catch (err) {

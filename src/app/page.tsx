...
import { 
  ...
  Brain, Menu, ChevronRight
} from "lucide-react";
import Link from "next/link";

// Social icons from different sources (using simple elements instead)
...
export default function LandingPage() {
  ...
  const [messages, setMessages] = useState([
    { role: "bot", content: "আসসালামু আলাইকুম! আমি FinNexus AI। আপনার আর্থিক লেনদেনের কী তথ্য দেব?" }
  ]);
  ...
  const handleSendMessage = async () => {
    ...
    setTimeout(() => {
      setMessages((prev) => [...prev, { 
        role: "bot", 
        content: "আপনার তথ্যটি প্রসেস করা হচ্ছে... (API Integrated via FinNexus Core)" 
      }]);
    }, 1000);
  };
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 50 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 50 }} className="absolute bottom-28 right-0 w-[350px] md:w-[450px] h-[600px] bg-slate-950/95 border border-white/10 rounded-[3.5rem] shadow-2xl overflow-hidden flex flex-col backdrop-blur-3xl">
              ...
              <div className="p-4 border-t border-white/10">
                <div className="flex gap-2">
                  <input type="text" value={chatInput} onChange={(e) => setChatInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSendMessage()} placeholder="Ask me anything..." className="flex-1 p-3 bg-white/10 rounded-xl text-white placeholder:text-slate-500 focus:outline-none focus:border-emerald-500" />
                  <button onClick={handleSendMessage} className="p-3 bg-emerald-500 rounded-xl text-black"><Send size={20} /></button>
                </div>
              </div>
            </motion.div>
          )}
